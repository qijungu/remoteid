...
void SITL_State::_usage(void)
{
    printf("Options:\n"
           ...
           "\t--remoteid               set remote id (20 bytes in 40 hex digits, no 0x), optional, default set to 616263646566303132333435363738397778797A\n"
           "\t--portbr                 set port to broadcast, optional, default set to 7100\n"
        );
}
...
void SITL_State::_parse_command_line(int argc, char * const argv[])
{
    ...
    uint8_t remoteid[20];            // for remoteid
    uint16_t broadcast_port = 7100;  // for remoteid
    ...
    enum long_options {
        ...
        CMDLINE_START_TIME,
        CMDLINE_SYSID,
        CMDLINE_REMOTEID,   // for remoteid
        CMDLINE_PORTBR,     // for remoteid
    };
    ...
    const struct GetOptLong::option options[] = {
        ...
        {"remoteid",        true,   0, CMDLINE_REMOTEID},  // for remoteid
        {"portbr",          true,   0, CMDLINE_PORTBR},    // for remoteid
        {0, false, 0, 0}
    };
    ...
    while (!is_replay && (opt = gopt.getoption()) != -1) {
        ...
        case CMDLINE_REMOTEID:   // for remoteid
            char d[3];
            for (int k=0; k<20; k++) {
                memcpy(d, &gopt.optarg[k*2], 2);
                d[2] = '\0';
                remoteid[k] = (uint8_t)strtoul(d, NULL, 16);
            }
            break;
        case CMDLINE_PORTBR:     // for remoteid
            broadcast_port = atoi(gopt.optarg);
            break;
        default:
            _usage();
            exit(1);
    ...
    for (uint8_t i=0; i < ARRAY_SIZE(model_constructors); i++) {
        if (strncasecmp(model_constructors[i].name, model_str, strlen(model_constructors[i].name)) == 0) {
            ...
            sitl_model->set_remoteid(remoteid);
            sitl_model->set_broadcast_port(broadcast_port);
            _synthetic_clock_mode = true;
            break;
        }
    }
