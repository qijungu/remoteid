...
int ecfp2_verify_coordinates(const fp2_t x, const fp2_t y)  {
	// y² = x^3 + a'x + b'
	fp2_t r0, r1;
	...
	return fp2_compare(r0, r1);
}

int ecfp2_verify_homogeneous_projective_coordinates(const fp2_t x, const fp2_t y, const fp2_t z) {
 ...
 */
int ecfp2_compare_proj(ecpoint_fp2_proj *a, const ecpoint_fp2_proj *b) {
...
/**
 * Compares two points of an elliptic curve
 * @param a the first point
 * @param b the second point
 * @return 0 if equal, 1 otherwise
 */
int ecfp2_compare_std(ecpoint_fp2 *a, const ecpoint_fp2 *b) {
    return a->infinity != b->infinity ||
            fp2_compare(a->x, b->x) ||
            fp2_compare(a->y, b->y);
}
...
void ecfp2_rand_std(ecpoint_fp2 *res) {
  ...
  ecfp2_mul(res, &ECFP2_GENERATOR, r);
}

/**
 * Compute the y coordinate from the x coordinate of a point
 * @param res     y coordinate
 * @param x       x coordinate
 * @return   0 if y exists, otherwise 1.
 */
int ecfp2_get_y(fp2_t res, const fp2_t x)  {
	// y² = x^3 + a'x + b'
	fp2_t r0, r1;
	...
	fp2_sqrt(res, r1);
	return ecfp2_verify_coordinates(x, res);
}

/**
 * Get a point from the x coordinate of a point
 * @param x       x coordinate
 * @param res     the point
 * @return   0 if y exists, otherwise 1.
 */
int ecfp2_get_point(ecpoint_fp2* res, const fp2_t x) {
    fp2_t y;
    int v = ecfp2_get_y(y, x);
    // cannot recover a point from x, so not a valid point
    if (v != 0) return v;
