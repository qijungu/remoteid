...
struct BasicMessageData {
    unsigned uaType : 4;  // enum RID_ID_Type
    unsigned idType : 4;  // enum RID_UA_Type
    ...
    public:
        ...
        BasicIDMessage(
           ...
           const uint8_t uasId[RIDLEN]
            ...
            for (int i=0; i<RIDLEN; i++) basicIDMessage.uasId[i] = uasId[i];
            data_len = sizeof(struct BasicMessageData);
            data = (uint8_t*)&basicIDMessage;
        };

        BasicIDMessage(const uint8_t* d) {
            assert(sizeof(struct BasicMessageData)==24);
            memset(&basicIDMessage, 0, sizeof(struct BasicMessageData));
