...
/**
 * H1
 * @param res, the hash result
 * @param m,   the message to hash in bytes
 * @param len, the length of message in bytes
 */
void H1(bigint_t res, const byte* m, int len) {
    byte hash[HASH_BYTES];
    byte _m[len+1];
 ...
 * @param len, the length of message in bytes
 */
void H2(bigint_t res, const byte* m, int len) {
    byte hash[HASH_BYTES];
    byte _m[len+1];
...
/**
 * Updates the given Hash state with the given message.
...
void hash_update_message(hashState *state, const char *message)
{
  ...
  unsigned int message_bytes = strlen(message);
  if(message_bytes < BLOCK_BYTES) {
    memset(block, 0, BLOCK_BYTES);
    memcpy(block, message, message_bytes);
    hash_update(state, block, BLOCK_BITS);
  } else {
    unsigned int i = 0;
    for(i = 0; message_bytes > BLOCK_BYTES; i++) {
      memset(block, 0, BLOCK_BYTES);
      memcpy(block, (message + i*BLOCK_BYTES), BLOCK_BYTES);
      hash_update(state, block, BLOCK_BITS);
      message_bytes -= BLOCK_BYTES;
    }
    memset(block, 0, BLOCK_BYTES);
    memcpy(block, (message + i*BLOCK_BYTES), message_bytes);
    hash_update(state, block, BLOCK_BITS);
  }
